...
class Painter extends StatefulWidget {
  ...
  Painter(this.painterController, {this.usePen = false})
...
class PainterState extends State<Painter> {
  ...
  void _onPenDown(PointerEvent start) {
    ...
    */
    ...
    widget.painterController._pathHistory.add(pos);
    //widget.painterController._notifyListeners();
    Offset pos2 = Offset(pos.dx, pos.dy + 2);
    widget.painterController._pathHistory.updateCurrent(pos2);
    widget.painterController._notifyListeners();
    //print("down: ${start.position}");
  }

  void _onPenMove(PointerEvent update) {
    Offset pos = (context.findRenderObject() as RenderBox)
        .globalToLocal(update.position);
    widget.painterController._pathHistory.updateCurrent(pos);
    widget.painterController._notifyListeners();
    //print("move: ${update.position}");
  }

  void _onPenUp(PointerEvent end) {
    Offset pos =
        (context.findRenderObject() as RenderBox).globalToLocal(end.position);
    widget.painterController._pathHistory.updateCurrent(pos);
    widget.painterController._pathHistory.endCurrent();
    widget.painterController._notifyListeners();
    //print("up: ${end.position}");
  }
}
